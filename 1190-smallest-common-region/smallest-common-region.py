class Solution(object):
    def findSmallestRegion(self, regions, region1, region2):
        ...
        # Check if region1 or region2 is not in the hierarchy map
        if region1 not in hierarchy_map or region2 not in hierarchy_map:
            return None

        # If region2 is a direct child of region1 or region1 has no parent
        if hierarchy_map[region2] == region1 or hierarchy_map[region1] is None:
            return region1
        # If region1 is a direct child of region2 or region2 has no parent
        ...
        # Collect all ancestors of region2 and find common ancestor
        ...
        return None  # If no common ancestor found
